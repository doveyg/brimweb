<?PHP 
$title = 'Batch Department Selector';
...
$hookReplace['calform'] = fStandardCal('batchrecipesplash', '[2004, 05, 18]');
fSetDates($startDate, $endDate, 30);
$bConn = odbc_connect('DRIVER={SQL Server};Server=INSQL2;Database=BatchHistory;', 'sa', 'woznium');
$queryBatchDepartment = 'select departmentfk, name, SUM(count) as count, train_id 
from (select train_id, COUNT(*) as count
...
if (@$_SESSION['id'] != 1 && isset($_SESSION['id'])) {
	$aQueryTrain[] = 'DepartmentFK is null';
	$queryBatchDepartment .= ' where ' . fOrThemReturn($_SESSION['permissions']['department'], 100, 'departmentfk', $aQueryTrain);
};
$queryBatchDepartment .= ' group by departmentfk, name, train_id';
$dataBatchDepartment = odbc_exec($bConn, $queryBatchDepartment);
while(odbc_fetch_row($dataBatchDepartment)) {
	if (!odbc_result($dataBatchDepartment, 1)) {
		$aUnused[odbc_result($dataBatchDepartment, 1)] = odbc_result($dataBatchDepartment, 4);
	};
	if (!isset($aBatchDepartments[odbc_result($dataBatchDepartment, 1)])) {
		$aBatchDepartments[odbc_result($dataBatchDepartment, 1)] = odbc_result($dataBatchDepartment, 3);
	} else {
		$aBatchDepartments[odbc_result($dataBatchDepartment, 1)] += odbc_result($dataBatchDepartment, 3);
	};
};
if (!empty($aBatchDepartments)) {
	asort($aBatchDepartments);
	$stdOut .= '<h2>Showing department data between ' . $startDate . ' and ' . $endDate . '</h2>';
	if (!empty($aUnused)) {
		$extraNotifications .= '<div class="ui-notif-error"><span class="icon-remove-sign"></span><b>Undefined Trains Please ';
		if (fCanSee(@$_SESSION['permissions']['page'][4] >= 200 && isset($_SESSION['edit']['departmentedit']))) {
			$extraNotifications .= '<a href="trainadmin.php">Fix</a>';
		} else {
			$extraNotifications .= 'Report';
		};
		$extraNotifications .= ':</b> ' . implode(', ', $aUnused) . '</div>';
	};
	...
	$aTicks = array();
	$i = 1;
	foreach($aBatchDepartments as $key => $array) {
		if ($key != NULL) {
			$data .= $sepB . '{ label: "' . $aDepartments[$key] . '", data: [ [ ' . $array . ', ' . $i . ', "' . $aDepartments[$key] . '", "batchrecipe.php' . fQueryString(['include' => ['department' => $key]]) . '" ] ] }';
		} else {
			$data .= $sepB . '{ label: "Undefined", data: [ [ ' . $array . ', 0, "Undefined", "batchrecipe.php' . fQueryString(['include' => ['department' => NULL]]) . '" ] ] }';
...
odbc_close($bConn);
$hookReplace['help'] = $helptext['standardcal'] . $helptext['barhover'] . '<a href="#">Bar Graph Clicking</a><div>Clicking a Bar in the Bar Graph will take you to the selected departments page.</div>'
